...
async def extract_token_from_header(authorization: Annotated[str | None, Header()] = None) -> str:
   if not authorization:
       raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=returnsdata.error_msg("Missing authorization header", ERROR))
   try:
       scheme, token = authorization.split()
       if scheme.lower() != "bearer":
           raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=returnsdata.error_msg("Invalid authorization scheme", ERROR))
       return token
   except ValueError:
       raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=returnsdata.error_msg("Invalid authorization format", ERROR))
