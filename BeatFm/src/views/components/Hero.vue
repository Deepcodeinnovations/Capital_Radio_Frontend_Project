        ...
        <div class="container mx-auto px-4 md:px-6">
          <div class="max-w-4xl mx-auto text-center">
            ...
            <div class="flex flex-col sm:flex-row gap-3 justify-center mb-8">
              ...
              >
                <div class="absolute -inset-0.5 bg-red-600  blur opacity-60 group-hover:opacity-80 transition duration-300"></div>
                <div class="relative bg-red-600 px-6 py-3 text-white font-bold flex items-center justify-center space-x-2 transition-all duration-300">
                  <div v-if="isLoading" class="w-4 h-4 border-2  border-white border-t-transparent animate-spin"></div>
                  <Volume2 v-else-if="isPlaying" :size="18" class="animate-bounce" />
                  <Play v-else :size="18" />
                  <span class="tracking-wide text-sm" v-if="isLoading">CONNECTING</span>
                  <span class="tracking-wide text-sm" v-else-if="isPlaying">LISTENING LIVE</span>
                  <span class="tracking-wide text-sm" v-else>TUNE IN NOW</span>
                </div>
              </button>
              ...
              >
                <ExternalLink :size="18" class="group-hover:scale-110 transition-transform duration-300" />
            ...
            <div class="grid grid-cols-3 gap-4 mb-8 max-w-lg mx-auto">
              <div class="text-center">
                <div class="text-2xl md:text-3xl font-bold text-white mb-1">96.3</div>
                <div class="text-xs md:text-sm text-white/80">MHz Frequency</div>
              </div>
              <div class="text-center">
              ...
              <div class="text-center">
                <div class="text-2xl md:text-3xl font-bold text-white mb-1">{{ currentStation?.listeners || '1M' }}+</div>
...
<div v-if="carouselItems && carouselItems.length > 1" class="absolute inset-y-0 left-0 right-0 z-30 pointer-events-none">
  <!-- Left Control -->
  <div class="absolute left-4 top-1/2 transform -translate-y-1/2 pointer-events-auto">
    <button
      @click="prevSlide"
  ...
  <!-- Right Control -->
  <div class="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-auto">
    <button
      @click="nextSlide"
      class="p-3 bg-red-600/80 backdrop-blur-md text-white hover:bg-red-600 transition-all duration-300 rounded-full border border-red-500/50 shadow-lg"
    >
  ...
  <div class="absolute bottom-8 left-1/2 transform -translate-x-1/2 pointer-events-auto">
    <div class="flex space-x-2 bg-red-600/80 backdrop-blur-md px-4 py-2 rounded-full border border-red-500/50">
      <button
        v-for="(slide, index) in carouselItems"
        :key="slide?.id || index"
...
const carouselItems = computed(() => {
  ...
  if (items.length === 0) {
    return [{
      id: 'fallback',
      title: 'Beat FM 96.3',
      description: 'Ekintabuli Ky\'Omuziki - Uganda\'s Hottest Beats & Entertainment',
