    ...
    <section class="relative min-h-[70vh] flex items-center justify-center overflow-hidden" style="margin-top: 5rem !important;">
      ...
      <div class="relative z-10 container mx-auto px-6 text-center">
        ...
        <div class="inline-block bg-red-600 text-white px-6 py-2 mb-8 transform -rotate-1">
          <span class="font-black text-sm tracking-widest uppercase">RADIO PROGRAMS</span>
        </div>
        ...
        <p class="text-xl text-white/90 mb-12 max-w-3xl mx-auto leading-relaxed">
          Listen to recorded radio programs from {{ station?.name || 'Beat FM' }}
        </p>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12 max-w-4xl mx-auto">
          <div class="bg-red-600 border-4 border-red-600 p-4 text-center transform hover:scale-105 transition-transform">
            <div class="text-3xl font-black text-white mb-1">{{ sessionRecordings?.total || 0 }}</div>
            <div class="text-xs font-bold text-white uppercase tracking-wider">EPISODES</div>
          </div>
          <div class="bg-white/90 backdrop-blur-sm border-4 border-white p-4 text-center transform hover:scale-105 transition-transform">
            <div class="text-3xl font-black text-black mb-1">{{ uniquePrograms.length }}</div>
            <div class="text-xs font-bold text-black uppercase tracking-wider">PROGRAMS</div>
          </div>
          <div class="bg-black/80 backdrop-blur-sm border-4 border-black p-4 text-center transform hover:scale-105 transition-transform">
            <div class="text-3xl font-black text-white mb-1">{{ totalDuration }}</div>
            <div class="text-xs font-bold text-white uppercase tracking-wider">HOURS</div>
          </div>
          <div class="bg-white/90 backdrop-blur-sm border-4 border-red-600 p-4 text-center transform hover:scale-105 transition-transform">
            <div class="text-3xl font-black text-red-600 mb-1 flex items-center justify-center gap-2">
              {{ todayEpisodes }}
              <div v-if="todayEpisodes > 0" class="w-2 h-2 bg-red-600 animate-pulse"></div>
            </div>
            <div class="text-xs font-bold text-red-600 uppercase tracking-wider">TODAY</div>
          </div>
        </div>
      ...
      <div class="absolute bottom-0 left-0 w-full overflow-hidden" style="transform: translateY(1px);">
        <svg class="w-full h-auto" preserveAspectRatio="none" viewBox="0 0 1200 120" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <linearGradient id="showWaveGradient" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" style="stop-color:#DC2626;stop-opacity:1" />
              <stop offset="50%" style="stop-color:#000000;stop-opacity:1" />
              <stop offset="100%" style="stop-color:#DC2626;stop-opacity:1" />
            </linearGradient>
          </defs>
          <path d="M0,60 C200,100 400,20 600,60 C800,100 1000,20 1200,60 L1200,120 L0,120 Z" fill="url(#showWaveGradient)"></path>
    ...
    <div class="bg-gray-50 relative" id="shows-content">
      <div class="max-w-7xl mx-auto px-6 py-12 relative z-10">
        ...
        <div class="mb-12">
          ...
          <div class="flex flex-wrap justify-center gap-3 mb-8">
            <button
              ...
              :class="quickFilter === 'today' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-black border-black hover:bg-black hover:text-white'"
            ...
            <button
              ...
              :class="quickFilter === 'yesterday' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-black border-black hover:bg-black hover:text-white'"
              class="px-6 py-3 font-black transition-all border-4 uppercase tracking-wider"
            >
              YESTERDAY
            </button>
            <button
              ...
              :class="quickFilter === 'week' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-black border-black hover:bg-black hover:text-white'"
              class="px-6 py-3 font-black transition-all border-4 uppercase tracking-wider"
            >
              THIS WEEK
            </button>
        ...
        <div v-else-if="filteredShows.length > 0">
          ...
          <div v-if="featuredShows.length > 0" class="mb-12">
            <div class="flex items-center gap-4 mb-8">
              ...
              <h2 class="text-3xl font-black text-black uppercase tracking-wider">FEATURED SHOWS</h2>
              ...
              >
                <div class="relative h-48 overflow-hidden">
                  ...
                  <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    ...
                    >
                      <Pause v-if="currentlyPlaying?.id === show.id && isAudioPlaying" class="w-8 h-8" />
                  ...
                  <div class="absolute top-4 right-4">
                    <span v-if="show.recording_status === 'completed'" class="bg-red-600 text-white px-3 py-1 text-xs font-black tracking-wider uppercase">
                      AVAILABLE
                    </span>
                    <span v-else class="bg-black text-white px-3 py-1 text-xs font-black tracking-wider uppercase">
                    ...
                    <div class="flex items-center space-x-2">
                      <div class="flex -space-x-1">
                        <div v-for="host in show.hosts?.slice(0, 3)" :key="host.id" 
                          ...
                          <img v-if="host.image_url" :src="host.image_url" :alt="host.name" class="w-full h-full object-cover">
                          <span v-else class="text-xs font-black text-white">{{ host.name.charAt(0) }}</span>
                        </div>
                      </div>
                      <span class="text-sm font-black text-black uppercase">{{ getHostsText(show.hosts) }}</span>
                    </div>
          ...
          <div class="space-y-4 mb-8">
            <div class="flex items-center gap-4 mb-8">
              ...
              <h2 class="text-3xl font-black text-black uppercase tracking-wider">ALL EPISODES</h2>
            ...
            >
              <div class="flex items-start space-x-4">
                <div class="flex-shrink-0">
                  <div class="w-24 h-24 border-2 border-black overflow-hidden relative group/play">
                    <img 
                      :src="show.program?.image_url || getDefaultShowImage(show.program?.title)" 
                      :alt="show.program?.title" 
                    ...
                    <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      ...
                      >
                        <Pause v-if="currentlyPlaying?.id === show.id && isAudioPlaying" class="w-5 h-5" />
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center space-x-3 mb-2">
                    ...
                    <span v-if="show.recording_status === 'completed'" class="text-xs px-2 py-1 bg-red-600 text-white font-black uppercase tracking-wider">
                      AVAILABLE
                    </span>
                    <span v-else class="text-xs px-2 py-1 bg-black text-white font-black uppercase tracking-wider">
                      {{ show.recording_status.toUpperCase() }}
                    </span>
                  </div>
              ...
              <div v-if="currentlyPlaying?.id === show.id" class="mt-4 pt-4 border-t-4 border-black">
                <div class="flex items-center space-x-4">
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between text-xs text-black font-black mb-1 uppercase">
        ...
        <div v-else class="text-center py-20">
          ...
          <h3 class="text-2xl font-black text-black mb-4 uppercase tracking-wider">NO SHOWS FOUND</h3>
...
const totalDuration = computed(() => {
  const total = recordingsData.value.reduce((sum, recording) => sum + (recording.duration_minutes || 0), 0);
  return Math.round(total / 60) + 'H';
});

const todayEpisodes = computed(() => {
  const today = new Date().toISOString().split('T')[0];
  return recordingsData.value.filter(recording => 
...
const featuredShows = computed(() => {
  return filteredShows.value.filter(show => show.is_featured || show.recording_status === 'completed').slice(0, 4);
});
...
const getHostsText = (hosts) => {
  ...
  if (hosts.length === 2) return `${hosts[0].name.toUpperCase()} & ${hosts[1].name.toUpperCase()}`;
