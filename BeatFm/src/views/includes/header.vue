<template>
  <div>
    ...
    <header class="fixed top-0 left-0 right-0 z-40 bg-red-600 border-b-2 border-red-700 shadow-lg">
      <div class="container mx-auto px-4 md:px-6">
        <div class="flex items-center justify-between h-20 md:h-24 lg:h-20">
          
          <!-- Logo Section -->
          <div class="flex items-center">
            <div class="h-12 w-32 sm:h-14 sm:w-36 md:h-16 md:w-40 lg:h-18 lg:w-48">
              <img 
                src="/logobeat.png" 
          ...
          <div class="flex items-center space-x-3 md:space-x-4">
            ...
            >
              <div :class="[
                'bg-white hover:bg-gray-100 px-4 md:px-6 py-2 md:py-3 text-black font-bold flex items-center space-x-2 transition-all duration-300 hover:scale-105 border-2 border-black shadow-lg',
                isLoading ? 'cursor-not-allowed opacity-75' : ''
              ]">
                <div v-if="isLoading" class="w-4 h-4 border-2 border-black border-t-transparent animate-spin"></div>
                <Volume2 v-else-if="isPlaying" :size="18" class="animate-pulse" />
                <Play v-else :size="18" />
                <span class="tracking-wider text-sm md:text-base font-black uppercase" v-if="isLoading">CONNECTING</span>
                <span class="tracking-wider text-sm md:text-base font-black uppercase" v-else-if="isPlaying">LIVE</span>
                <span class="tracking-wider text-sm md:text-base font-black uppercase" v-else>LISTEN</span>
              </div>
            </button>
            ...
            >
              <div v-if="isLoading" class="w-5 h-5 border-2 border-gray-600 border-t-transparent animate-spin"></div>
    ...
    <!-- Full Screen Menu -->
    <div v-if="isOpen" class="fixed inset-0 z-50 bg-gradient-to-br from-red-900 via-black to-red-900">
      <div class="absolute inset-0 bg-black/60 backdrop-blur-sm"></div>
      
      <div class="relative container mx-auto px-4 md:px-6 h-full flex items-center justify-center">
        ...
        <div class="text-center max-w-4xl mx-auto">
          ...
          <div class="mb-8 md:mb-12">
            ...
            >
              <div v-if="isLoading" class="w-6 h-6 border-2 border-current border-t-transparent animate-spin"></div>
          ...
          <!-- Social Media -->
          <div>
            <h6 class="text-white font-bold mb-4 md:mb-6 text-base md:text-lg uppercase tracking-wider">Follow Beat FM</h6>
